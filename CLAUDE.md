...
- Frontend is vanilla HTML/CSS/JS (deliberate — this is the user's JS learning ground, guide new concepts step by step in chat rather than editing their files directly — see workflow note below). `templates/index.html` is the static shell: a `.entete` div holds the logo (`static/images/logo.png`, `position: absolute` anchored to `.entete`'s `position: relative`) and the `<h1>` title, plus two Google Fonts `<link>` tags (Lobster Two for the title, Germania One for the subtitle — chosen over a locally-installed-only font so they render for every visitor), plus a `.barre-outils` div (currently `display: flex` only, no dedicated styling yet) holding the cinema filter (`#bouton-filtre` toggles a `.liste-cinemas` checkbox list, all checked by default), a sort `<select id="tri">` (options: default / release date ascending / descending), and the `<input type="date" id="jour">` day-picker. `static/script.js` fetches `/api/seances` (optionally `?jour=...` when the day-picker's `change` event fires) into `dernieresDonnees` (module-level cache, avoids refetching on filter/sort changes), then `mettreAJourAffichage()` is the central render path: `obtenirFilmsFiltres()` keeps only films present in a checked cinema, `trierFilms()` sorts by `dateSortie` (films with a `null` date always sort first, regardless of direction — deliberate "when in doubt, surface it" choice) unless `tri` is `"defaut"`, then `afficherFilms()` renders the grid (poster + title) via `document.getElementById("seances").innerHTML` and rewires the per-film click listeners (`activerClics()`, needed again each render since `innerHTML` destroys the old DOM nodes) that toggle a hidden per-cinema showtimes list (`style.display`), each showtime annotated with VF/VO (from `diffusionVersion`) and projection format (from `projection`/`experience`). `static/style.css` implements the "LUMI" palette (navy background, turquoise/cream/gold accents): a responsive `display: grid` film grid, card hover effect (`transform`/`box-shadow`/`transition`), styled headings/lists.

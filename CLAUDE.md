...
`/api/seances` only returns today's showtimes. Planned next: a global day-picker at the top of the page that refetches `/api/seances` with a chosen date (backend needs a date param, via `day_shift` or an explicit date) and re-renders the grid — replaces an earlier idea of stacking every day's showtimes inside each film card, judged too visually heavy. See roadmap doc for detail.
